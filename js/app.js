...
async function downloadOutputImage() {
  ...
  if (typeof domtoimage === "undefined") {
    ...
    alert(
      "Image generation library not available. Please refresh the page and try again."
    );
  ...
  domtoimage
    ...
    .catch(function (error) {
      ...
      if (
        ...
        const externalLinks = cleanOutput.querySelectorAll(
          'link[rel="stylesheet"]'
        );
  ...
  function processGeneratedBlob(blob, scale) {
    ...
    img.onerror = function () {
      ...
      alert(
        "There was an error processing the generated image. Please try again."
      );
      hideLoadingIndicator();
    };
  }
...
function toggleHistoryPanel() {
  ...
  tab.setAttribute(
    "aria-label",
    isOpen ? "Open chat history" : "Close chat history"
  );
...
$(document).ready(function () {
  ...
  function renderCharacterDropdown() {
    ...
    dropdown.html(
      list
        ...
        <span class="character-name-select">${$("<div>")
          .text(name)
          .html()}</span>
        <button class="remove-character-btn" data-name="${$("<div>")
          .text(name)
          .html()}" style="background: none; border: none; color: #c00; font-size: 16px; cursor: pointer;">&times;</button>
      </div>`
        )
        .join("")
    );
  ...
  $("#characterNameInput").val(
    localStorage.getItem("chatlogCharacterName") || ""
  );
